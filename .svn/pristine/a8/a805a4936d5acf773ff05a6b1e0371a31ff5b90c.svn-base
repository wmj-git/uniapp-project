...
<script>     
	...
	export default {
		...
		onShow(){
			if(uni.getStorageSync('from_mid')!=="undefined"&&uni.getStorageSync('from_mid') !==undefined&&uni.getStorageSync('from_mid') !==''){
		...
		onLoad(options) {
			...
			if(scene!=="undefined"&&scene !==undefined&&scene !==''){				
				...
				app.wxLogin().then(res => {
					...
					console.log(token)
					this.up_from_midAPI(token,from_mid);
				})				
				this.from_infoAPI();				
				this.fromIdShow=true	
			}	
	
			if(options.from_mid	!=="undefined"&&options.from_mid !==undefined&&options.from_mid !==''){				
				uni.clearStorage();				
				var from_mid = options.from_mid
				uni.setStorageSync('from_mid',from_mid)		
				app.wxLogin().then(res => {
					var token = uni.getStorageSync('token');
					this.up_from_midAPI(token,from_mid);
		   ...
		   if(app.globalData.token=="undefined"&&app.globalData.token !==undefined){
		...
		methods: {		
			...
			async up_from_midAPI(token,from_mid){
				
				const res = await this.$myRequset({
				    url: '/api', 
					method:'POST',
				    data: {
				        operate: 'MemberCenter.up_from_mid',
						token:token,
						from_mid:from_mid
				    }
				});
				//console.log("from_mid修改")
				//console.log(res)			
