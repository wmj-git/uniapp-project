<template>
	<view class="prohibition">
		<view class="demo" :style="'height:' +  demo.height + 'px;' + 'padding-top:' + demo.top + 'px;padding-bottom:10rpx'">
			...
			<view class="clr_fff">预约订单详情</view>
		</view>
		...
		<view class="content">
			<view class="relative">
				<view class="nurse_box">
					<view class="nurse_item">
						<view class="person_info info_color nickname">买家姓名： <text class="nickname_text">{{ detail.nickname }}</text></view>
					</view>
					<view class="nurse_item">
						<view class="info info_color">被护理人： <text class="name">{{ detail.caregivers.name }}</text>
							<text>{{ detail.caregivers.sex}}</text>
							<text>{{ detail.caregivers.age }}岁</text></view>
					</view>
					<view class="nurse_item">
						<view class="person_info info_color">联系电话： <text>{{ detail.caregivers.phone }}</text></view>
					</view>
					<view class="nurse_item">
						<view class="person_info info_color address_letter">联系地址： <text class="sub_text">{{detail.caregivers.address}}</text></view>
					</view>
					<view class="nurse_item">
						<view class="person_info info_color">护理内容： <text>居家·{{detail.title}}</text></view>
					</view>
					<view class="nurse_item">
						<view class="person_info info_color time">护理开始时间： <text class="date_end">{{detail.start_time}}</text></view>
						<view class="person_info info_color time">护理结束时间： <text class="date_end">{{detail.end_time}}</text></view>
					</view>
				</view>
				<view>
					<view class="uni-flex my_worker" @tap="toDetail">
						<view class="header">
					...
					<view class="hugong">
						...
						<view class="btn">
							<button class="btn_pj verify_btn" v-show="finsh_confirm == true" hover-class="active" @tap="verifyFinish">确认完工</button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async finishFn() {
				const res = await this.$myRequset({
					...
					data: {
						...
						token: this.token,
						appointment_id: this.params.appoint_id,
			...
			toDetail(index) {
				uni.navigateTo({
					url: "../workerDetail/workerDetail?order_no=" + this.params.order_no + "&caregivers_id=" + this.params.caregivers_id +
						"&nurse_id=" + this.params.nurse_id + "&appoint_id=" + this.params.appoint_id + "&status=" + this.order_status
				})
			},
			async getNurseDetail() {
				...
				this.detail.start_time =  Util.timestampToTime(this.detail.start_time);
				this.detail.end_time =  Util.timestampToTime(this.detail.end_time);
			...
			async confirmFn() {
				if (!this.text_val) {
					...
					if (res.data.code === 1) {
						...
						// 评论提交后刷新当前页面
						uni.redirectTo({
							url: "../workerAllDetail/workerAllDetail?order_no=" + this.params.order_no + "&caregivers_id=" + this.params.caregivers_id +
								"&nurse_id=" + this.params.nurse_id + "&appoint_id=" + this.params.appoint_id
						});
					};
				}
			}
...
<style lang="scss">
	...
	.header {
		...
		height: 135rpx;
		border-radius: 50%;
		background: url("https://test134.vrapp.chot.cn/mini/picture/headimg_default.png") no-repeat center;
	}
